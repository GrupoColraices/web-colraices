...
export const BarSearch = ({ visibility = false, scrolling = 450 }) => {
    ...
    const pageFavorite = pathName === '/tour-de-la-vivienda/favoritos'
    const pagePropery = pathName.includes('/tour-de-la-vivienda/inmueble')
    ...
    const setData = (e) => {
        ...
        if (inputs.city != "0" && labelFilter.length > 0 && isFound) {
            ...
            router.push(`/tour-de-la-vivienda/filtrados/${filterSegments.join('/')}?${filterQuery.join('&')}`);
        } else {
            e.preventDefault();
            setValidate(true);
